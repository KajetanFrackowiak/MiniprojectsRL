        ...
        for state in range(self.env.observation_space.n):
            for action in range(self.env.action_space.n):
                ...
                for tgt_state, count in target_counts.items():
                    ...
                    # not only self.values[(tgt_state)], but else best_action
...
if __name__ == "__main__":
    ...
    wandb.init(project="bellman", name="q-learning")
    ...
    while True:
        ...
        if reward > best_reward:
            print(f"{iter_no}: Best reward updated, {best_reward:.3f} -> {reward:.3}")
            best_reward = reward
        if reward > 0.8:
